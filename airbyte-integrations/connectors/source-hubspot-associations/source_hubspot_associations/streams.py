...
from source_hubspot_associations.constants import OAUTH_CREDENTIALS, PRIVATE_APP_CREDENTIALS
from source_hubspot_associations.errors import HubspotAccessDenied, HubspotInvalidAuth, HubspotRateLimited, HubspotTimeout, InvalidStartDateConfigError
from source_hubspot_associations.helpers import APIv1Property, APIv3Property, GroupByKey, IRecordPostProcessor, IURLPropertyRepresentation, StoreAsIs
...
class Stream(HttpStream, ABC):
    ...
    def _flat_associations(self, records: Iterable[MutableMapping]) -> Iterable[MutableMapping]:
        ...
        "contacts": [{"id": "201", "type": "company_to_contact"}, {"id": "251", "type": "company_to_contact"}]
        """
        for record in records:
            if "associations" in record:
                associations = record.pop("associations")
                for name, association in associations.items():
                    record[name.replace(" ", "_")] = [row for row in association.get("results", [])]
            yield record
...
class ContactsMergedAudit(Stream):
    ...
    def get_json_schema(self) -> Mapping[str, Any]:
        ...
        source_hubspot_associations/schemas/contacts_merged_audit.json
        """
...
class Tickets(CRMSearchStream):
    entity = "ticket"
    associations = ["contacts", "deals", "companies", "Liegenschaften"]
    primary_key = "id"
    scopes = {"tickets"}
    last_modified_field = "hs_lastmodifieddate"
